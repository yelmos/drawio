...
Sidebar.prototype.addGeneralPalette = function(expand)
{
	 ...
	 */
	var fnsDarwin = [
		this.createVertexTemplateEntryDarwin('dashed=1;fontSize=19;ellipse;whiteSpace=wrap;html=1;', 120, 80, 'input', 'input', null, null, 'oval ellipse state', "input"),
		this.createVertexTemplateEntryDarwin('fontSize=15;rounded=1;whiteSpace=wrap;html=1;', 120, 60, 'Layer', 'Layer', null, null, 'rounded rect rectangle box', "layer"),
		...
		this.addEntry(lineTags + 'edge title multiplicity', mxUtils.bind(this, function()
		{
	    	...
	    	var cell0 = new mxCell('', new mxGeometry(0, 0, 0, 0), 'edgeLabel;resizable=0;html=1;align=center;verticalAlign=middle;', "weight");
	    	cell0.geometry.relative = true;
	    	cell0.setConnectable(false);
	    	cell0.vertex = true;
	    	edge.insert(cell0);
	    	
	    	var cell1 = new mxCell('', new mxGeometry(-1, 0, 0, 0), 'edgeLabel;resizable=0;html=1;align=left;verticalAlign=bottom;', "sourcenode");
	    	cell1.geometry.relative = true;
	    	cell1.setConnectable(false);
	    	cell1.vertex = true;
	    	edge.insert(cell1);
			
	    	var cell2 = new mxCell('', new mxGeometry(1, 0, 0, 0), 'edgeLabel;resizable=0;html=1;align=right;verticalAlign=bottom;', "targetnode");
	    	cell2.geometry.relative = true;
	    	cell2.setConnectable(false);
	    	cell2.vertex = true;
	    	edge.insert(cell2);
	    	
			return this.createEdgeTemplateFromCells([edge], 160, 0, 'Topology');
		})),
	];
	var fns = [
	...
	fns = darwin.enable?fnsDarwin:fns;
	this.addPaletteFunctions("snnmodelbuilditem", mxResources.get("snnmodelbuilditem"),  true, fns);
	this.setCurrentSearchEntryLibrary();
};
...
Sidebar.prototype.createItem = function(cells, title, showLabel, showTitle, width, height, allowCellsInserted)
{
	 ...
	 * darwin flow
	 */
	darwinFlowWeight = 185;
	darwinFlowHeight = 60;
	elt.style.width = darwinFlowWeight + 'px';
	elt.style.height = darwinFlowHeight  + 'px';
	...
	this.createThumb(cells, darwinFlowWeight, darwinFlowHeight, elt, title, showLabel, showTitle, width, height);
 ...
 * darwin flow fixed 
 */
Sidebar.prototype.createVertexTemplateEntryDarwin = function(style, width, height, value, title, showLabel, showTitle, tags, type)
{
	tags = (tags != null && tags.length > 0) ? tags : ((title != null) ? title.toLowerCase() : '');
	
	return this.addEntry(tags, mxUtils.bind(this, function()
 	{
 		return this.createVertexTemplateDarwin(style, width, height, value, title, showLabel, showTitle, true, type);
 	}));
}
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplateDarwin = function(style, width, height, value, title, showLabel, showTitle, allowCellsInserted, type)
{
	var cells = [new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style, type)];
	cells[0].vertex = true;
