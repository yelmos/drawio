		...
		*/
		...
		if (containsLabel) {
			...
			if (cell.type === 'connector') {
				...
				// mxUtils.write(label, mxResources.get('outputproperties'));
				// div.appendChild(label);
			}
			...
			this.panels.push(new ItemPropertiesPanel(this, ui, stylePanel))
...
/**
 *  darwin: network item properties set
 */
ItemPropertiesPanel = function (format, editorUi, container) {
	BaseFormatPanel.call(this, format, editorUi, container);
	this.init();
};

mxUtils.extend(ItemPropertiesPanel, BaseFormatPanel);

ItemPropertiesPanel.prototype.defaultStrokeColor = 'black';

ItemPropertiesPanel.prototype.init = function () {
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	var cell = graph.getSelectionCell();
	if (cell.type === 'input') {
		...
		console.log(cell.type);
		// this.container.appendChild(this.OutputPropertiesDialog(this.createPanel()));
	} else if (cell.type === 'connector') {
		console.log(cell.type);
		this.container.appendChild(this.ConnectorPropertiesDialog(this.createPanel()));
	}
}
 ...
 *  input properties
 */
ItemPropertiesPanel.prototype.InputPropertiesDialog = function (div) {
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	...
	mxUtils.write(layerName, mxResources.get('layername') + ' :');
	layerName.setAttribute('title', mxResources.get('layernamerule'));
	layerName.setAttribute('type', 'text');
	layerName.style.marginTop = "6px";
	layerName.style.marginBottom = '6px';
	layerName.style.marginLeft = "25px";
	layerName.style.width = '100%';
	layerName.style.height = '20px';
	layerName.style.boxSizing = 'border-box';

	var layerNameValue = document.createElement('input');

	layerNameValue.setAttribute('type', 'text');
	layerNameValue.style.width = '150px';
	layerNameValue.style.height = '20px';
	layerNameValue.style.boxSizing = 'border-box';
	layerNameValue.style.float = "right";
	layerNameValue.style.marginRight = "80px";
	layerNameValue.style.border = "soild";
	layerNameValue.style.borderWidth = "1.5px";
	if (properties.get('layername') === undefined) {
		layerNameValue.setAttribute('placeholder', 'input');
	} else {
		layerNameValue.value = properties.get('layername');
	}
 ...
 * layer properties
 */
ItemPropertiesPanel.prototype.LayerPropertiesDialog = function (div) {
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	cell = graph.getSelectionCell();
	...
	var layerNameValue = document.createElement('input');
	layerNameValue.setAttribute('type', 'text');
	layerNameValue.setAttribute('placeholder', mxResources.get('layernamerule'));
	layerNameValue.style.width = '150px';
	layerNameValue.style.height = '20px';
	layerNameValue.style.float = "right";
	layerNameValue.style.marginRight = "80px";
	...
	// neuron size
	var neuronSize = document.createElement('div');
	neuronSize.setAttribute('title', mxResources.get('neuronsizerule'));
	mxUtils.write(neuronSize, mxResources.get('neuronsize') + ' :');
	neuronSize.setAttribute('type', 'text');
	neuronSize.style.marginTop = "6px";
	neuronSize.style.marginBottom = '6px';
	...
	var neuronSizeValue = document.createElement('input');
	neuronSizeValue.setAttribute('type', 'text');
	neuronSizeValue.setAttribute('placeholder', mxResources.get('positiveInteger'));
	neuronSizeValue.style.width = '150px';
	neuronSizeValue.style.height = '20px';
	neuronSizeValue.style.boxSizing = 'border-box';
	neuronSizeValue.style.marginRight = "80px";
	...
	//neuron type
	var neuronType = document.createElement("div");
	neuronType.setAttribute("title", mxResources.get("neurontype"));
	mxUtils.write(neuronType, mxResources.get("neurontype") + " :");
	neuronType.setAttribute("type", "text");
	neuronType.style.marginTop = "6px";
	neuronType.style.marginBottom = '6px';
	neuronType.style.marginLeft = "25px";
	neuronType.style.width = "100%";
	neuronType.style.height = "20px";
	neuronType.style.boxSizing = "border-box";
	var neuronTypeSelect = document.createElement("select");
	neuronTypeSelect.style.width = '150px';
	neuronTypeSelect.style.height = "20px";
	neuronTypeSelect.style.marginRight = "80px";
	neuronTypeSelect.style.float = "right";
	neuronTypeSelect.style.border = "soild";
	neuronTypeSelect.style.borderWidth = "1.5px";
	var neuronTypeOption0 = document.createElement("option");
	mxUtils.write(neuronTypeOption0, 'CLIF');
	var neuronTypeOption1 = document.createElement("option");
	mxUtils.write(neuronTypeOption1, 'LIF');
	var neuronTypeOption2 = document.createElement("option");
	mxUtils.write(neuronTypeOption2, 'IF');
	neuronTypeOption2.setAttribute("selected", "selected");
	var neuronTypeOption3 = document.createElement("option");
	mxUtils.write(neuronTypeOption3, 'IZH');
	if (properties.get('neurontype') === 'CLIF') {
		neuronTypeOption0.setAttribute("selected", "selected");
	} else if (properties.get('neurontype') === 'LIF') {
		neuronTypeOption1.setAttribute("selected", "selected");
	} else if (properties.get('neurontype') === 'IF') {
		neuronTypeOption2.setAttribute("selected", "selected");
	} else if (properties.get('neurontype') === 'IZH') {
		neuronTypeOption3.setAttribute("selected", "selected");
	}
	// neuronTypeSelect.appendChild(neuronTypeOption0);
	...
	leakMode.setAttribute("title", mxResources.get("leakmoderule"));
	mxUtils.write(leakMode, mxResources.get("leakmode") + " :");
	leakMode.setAttribute("type", "text");
	leakMode.style.marginTop = "6px";
	leakMode.style.marginBottom = '6px'
	leakMode.style.marginLeft = "25px";
	leakMode.style.width = "100%";
	leakMode.style.height = "20px";
	leakMode.style.boxSizing = "border-box";
	var leakModeSelect = document.createElement("select");
	leakModeSelect.style.width = '150px';
	leakModeSelect.style.height = "20px";
	leakModeSelect.style.marginRight = "80px";
	leakModeSelect.style.float = "right";
	var leakModeOption1 = document.createElement("option");
	mxUtils.write(leakModeOption1, "0");
	var leakModeOption2 = document.createElement("option");
	mxUtils.write(leakModeOption2, "1");
	var leakModeOption3 = document.createElement("option");
	mxUtils.write(leakModeOption3, "-1")
	if (properties.get('leakmode') === '0') {
		leakModeOption1.setAttribute("selected", "selected");
	} else if (properties.get('leakmode') === '1') {
		leakModeOption2.setAttribute("selected", "selected");
	} else if (properties.get('leakmode') === '-1') {
		leakModeOption3.setAttribute("selected", "selected");
	}
	leakModeSelect.appendChild(leakModeOption1);
	leakModeSelect.style.display = "none";

	var leakModeOfIF = document.createElement("div");
	leakModeOfIF.style.width = "150px";
	leakModeOfIF.style.height = "20px";
	leakModeOfIF.style.marginRight = "80px";
	leakModeOfIF.style["float"] = "right";
	leakModeOfIF.style.boxSizing = "border-box";
	leakModeOfIF.style.borderColor = '#505759';
	leakModeOfIF.style.borderStyle = 'solid';
	leakModeOfIF.style.borderWidth =
		"1.5px";
	leakModeOfIF.style.fontSize = "14px";
	leakModeOfIF.style.fontFamily = "Helvetica,Arial,sans-serif";
	mxUtils.write(leakModeOfIF, "0");
	leakModeOfIF.style.display = "block";
	mxEvent.addListener(neuronTypeSelect, "change", function () {
		if (neuronTypeSelect.options[neuronTypeSelect.selectedIndex].value === "IF") {
			leakModeOfIF.style.display = "block";
			leakModeSelect.style.display = "none";
		} else {
			leakModeOfIF.style.display = "none";
			leakModeSelect.style.display = "block";
		}
	});
	leakMode.appendChild(leakModeOfIF);
	leakMode.appendChild(leakModeSelect);

	// leak value
	var leakValue = document.createElement('div');
	leakValue.setAttribute('title', mxResources.get('leakvaluerule'));
	mxUtils.write(leakValue, mxResources.get('leakvalue') + ' :');
	leakValue.setAttribute('type', 'text');
	leakValue.style.marginTop = "6px";
	leakValue.style.marginBottom = '6px';
	leakValue.style.marginLeft = "25px";
	leakValue.style.width = '100%';
	leakValue.style.height = '20px';
	leakValue.style.boxSizing = 'border-box';

	var leakValueValue = document.createElement('input');
	leakValueValue.setAttribute('type', 'text');
	leakValueValue.style.width = '150px';
	leakValueValue.style.height = '20px';
	leakValueValue.style.marginRight = "80px";
	leakValueValue.style.float = "right";
	leakValueValue.style.boxSizing = 'border-box';
	leakValueValue.style.border = "soild";
	leakValueValue.style.borderWidth = "1.5px";
	leakValueValue.style.display = "none";
	if (properties.get('leakvalue') === undefined) {
		leakValueValue.setAttribute('value', '0');
		leakValueValue.setAttribute('readonly', 'true');
	} else {
		leakValueValue.value = properties.get('leakvalue');
	}
	var leakValueOfIF = document.createElement("div");
	leakValueOfIF.style.width = "150px";
	leakValueOfIF.style.height = "20px";
	leakValueOfIF.style.marginRight = "80px";
	leakValueOfIF.style["float"] = "right";
	leakValueOfIF.style.boxSizing = "border-box";
	leakValueOfIF.style.borderColor = '#505759';
	leakValueOfIF.style.borderStyle = 'solid';
	leakValueOfIF.style.borderWidth =
		"1.5px";
	leakValueOfIF.style.fontSize = "14px";
	leakValueOfIF.style.fontFamily = "Helvetica,Arial,sans-serif";
	mxUtils.write(leakValueOfIF, "0");
	leakValueOfIF.style.display = "block";
	mxEvent.addListener(neuronTypeSelect, "change", function () {
		if (neuronTypeSelect.options[neuronTypeSelect.selectedIndex].value === "IF") {
			leakValueOfIF.style.display = "block";
			leakValueValue.style.display = "none";
		} else {
			leakValueOfIF.style.display = "none";
			leakValueValue.style.display = "block";
		}
	});
	leakValue.appendChild(leakValueOfIF);
	leakValue.appendChild(leakValueValue);


	// reset mode
	var resetMode = document.createElement("div");
	resetMode.setAttribute("title", mxResources.get("resetmoderule"));
	mxUtils.write(resetMode, mxResources.get("resetmode") + " :");
	resetMode.setAttribute("type", "text");
	resetMode.style.marginTop = "6px";
	resetMode.style.marginBottom = "6px";
	resetMode.style.marginLeft = "25px";
	resetMode.style.width = "100%";
	resetMode.style.height = "20px";
	resetMode.style.boxSizing = "border-box";
	var resetModeSelect = document.createElement("select");
	resetModeSelect.style.width = '150px';
	resetModeSelect.style.height = "20px";
	resetModeSelect.style.float = "right";
	resetModeSelect.style.marginRight = "80px";
	resetModeSelect.style.border = "soild";
	resetModeSelect.style.borderWidth = "1.5px";
	var resetModeOption1 = document.createElement("option");
	mxUtils.write(resetModeOption1, mxResources.get("resetModeOption1"));
	var resetModeOption2 = document.createElement("option");
	mxUtils.write(resetModeOption2, mxResources.get("resetModeOption2"));
	if (properties.get('resetmode') === mxResources.get("resetModeOption1")) {
		resetModeOption1.setAttribute("selected", "selected");
	} else if (properties.get('resetmode') === mxResources.get("resetModeOption2")) {
		resetModeOption2.setAttribute("selected", "selected");
	}
	resetModeSelect.appendChild(resetModeOption1);
	...
	mxUtils.write(vThreshold, mxResources.get('vthreshold') + ' :');
	vThreshold.setAttribute('type', 'text');
	vThreshold.style.marginTop = "6px";
	vThreshold.style.marginBottom = "6px";
	vThreshold.style.marginLeft = "25px";
	vThreshold.style.width = '100%';
	vThreshold.style.height = '20px';
	vThreshold.style.boxSizing = 'border-box';

	var vThresholdValue = document.createElement('input');
	vThresholdValue.setAttribute('type', 'text');
	vThresholdValue.setAttribute('placeholder', mxResources.get('positiveInteger'));
	vThresholdValue.style.marginRight = "80px";
	vThresholdValue.style.float = "right";
	vThresholdValue.style.width = '150px';
	vThresholdValue.style.height = '20px';
	vThresholdValue.style.boxSizing = 'border-box';
	vThresholdValue.style.border = "soild";
	vThresholdValue.style.borderWidth = "1.5px";
	if (properties.get('vthreshold') === undefined) {
		vThresholdValue.setAttribute('placeholder', '0');
	} else {
		vThresholdValue.value = properties.get('vthreshold');
	}
	vThreshold.appendChild(vThresholdValue);

	div.appendChild(layerName);
	div.appendChild(neuronSize);
	div.appendChild(neuronType);
	div.appendChild(leakMode);
	...
	var applyBtn = mxUtils.button(mxResources.get('apply'), function () {
		texts.push(layerNameValue.value + '\n' +
			'size: ' + neuronSizeValue.value + '\n' +
			'type: ' + neuronTypeSelect.options[neuronTypeSelect.selectedIndex].value);
		texts.push(layerNameValue.value);
		texts.push(neuronSizeValue.value);
		texts.push(neuronTypeSelect.options[neuronTypeSelect.selectedIndex].value);
		texts.push(leakModeSelect.options[leakModeSelect.selectedIndex].value);
...
ItemPropertiesPanel.prototype.OutputPropertiesDialog = function (div) {
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	cell = graph.getSelectionCell();
	...
	// layer name 
	var layerName = document.createElement('div');
	mxUtils.write(layerName, mxResources.get('output') + ' :');
	layerName.setAttribute('title', mxResources.get('outputrule'));
	layerName.setAttribute('type', 'text');
	layerName.style.marginTop = "6px";
	layerName.style.marginBottom = "6px";
	layerName.style.marginLeft = "25px";
	...
	layerNameValue.style.height = '20px';
	layerNameValue.style.float = "right";
	layerNameValue.style.marginRight = '100px';
	layerNameValue.style.boxSizing = 'border-box';
	layerNameValue.style.border = "soild";
	layerNameValue.style.borderWidth = "1.5px";
	if (properties.get('layername') === undefined) {
		layerNameValue.setAttribute('placeholder', 'output');
	} else {
		layerNameValue.value = properties.get("layername");
	}
	...
	// decoding type
	var decodingType = document.createElement('div');
	decodingType.setAttribute('title', mxResources.get('decodingtype'));
	mxUtils.write(decodingType, mxResources.get('decodingtype') + ' :');
	decodingType.setAttribute('type', 'text');
	decodingType.style.marginBottom = "6px";
	decodingType.style.marginTop = "6px";
	decodingType.style.marginLeft = "25px";
	decodingType.style.width = '100%';
	decodingType.style.height = '20px';
	decodingType.style.boxSizing = 'border-box';

	var decodingTypeSelect = document.createElement('select');
	decodingTypeSelect.setAttribute('type', 'text');
	decodingTypeSelect.style.width = '150px';
	decodingTypeSelect.style.height = '20px';
	decodingTypeSelect.style.marginRight = '100px';
	decodingTypeSelect.style.float = "right";
	decodingTypeSelect.style.boxSizing = 'border-box';
	decodingTypeSelect.style.border = "soild";
	decodingTypeSelect.style.borderWidth = "1.5px";
	var decodingTypeOption0 = document.createElement('option');
	mxUtils.write(decodingTypeOption0, 'spike counts');

	if (properties.get('decodingtype') === "spike counts") {
		decodingTypeOption0.setAttribute("selected", "selected");
	}
	decodingTypeSelect.appendChild(decodingTypeOption0);
	decodingType.appendChild(decodingTypeSelect);

	//target layer
	var targetLayer = document.createElement("div");
	targetLayer.setAttribute("title", mxResources.get("targetlayer"));
	mxUtils.write(targetLayer, mxResources.get("targetlayer") + " :");
	targetLayer.setAttribute('type', 'text');
	targetLayer.style.marginTop = "6px";
	targetLayer.style.marginBottom = "6px";
	targetLayer.style.marginLeft = "25px";
	targetLayer.style.width = '100%';
	targetLayer.style.height = '20px';
	targetLayer.style.boxSizing = 'border-box';

	var targetLayerValue = document.createElement('input');
	targetLayerValue.setAttribute('type', 'text');
	targetLayerValue.style.width = '150px';
	targetLayerValue.style.height = '20px';
	targetLayerValue.style.marginRight = '100px';
	targetLayerValue.style.float = "right";
	targetLayerValue.style.boxSizing = 'border-box';
	targetLayerValue.style.border = "soild";
	targetLayerValue.style.borderWidth = "1.5px";
	if (properties.get('targetlayer') === undefined) {
		targetLayerValue.setAttribute('placeholder', '');
	} else {
		targetLayerValue.value = properties.get('targetlayer');
	}
	targetLayer.appendChild(targetLayerValue);

	div.appendChild(layerName);
	div.appendChild(decodingType);
	div.appendChild(targetLayer);
	// newProp.appendChild(nameInput);
	// top.appendChild(newProp);
	// div.appendChild(top);
	names = ['layername', 'decodingtype', 'targetlayer'];
	var addBtn = mxUtils.button(mxResources.get('addProperty'), function () {
		var name = nameInput.value;
	...
	var applyBtn = mxUtils.button(mxResources.get('apply'), function () {
		texts.push(layerNameValue.value);
		texts.push(decodingTypeSelect.options[decodingTypeSelect.selectedIndex].value);
		texts.push(targetLayerValue.value);
		try {
			ui.hideDialog.apply(ui, arguments);
...
ItemPropertiesPanel.prototype.ConnectorPropertiesDialog = function (div) {
	var ui = this.editorUi;
	var graph = ui.editor.graph;
	cell = graph.getSelectionCell();
	var value = graph.getModel().getValue(cell);
	...
	// connector name 
	var connectorName = document.createElement('div');
	mxUtils.write(connectorName, mxResources.get('connectorname') + ' :');
	connectorName.setAttribute('title', mxResources.get('connectornamerule'));
	connectorName.setAttribute('type', 'text');
	connectorName.style.marginTop = "6px";
	connectorName.style.marginBottom = "6px";
	connectorName.style.marginLeft = "25px";
	connectorName.style.width = '100%';
	connectorName.style.height = '20px';
	connectorName.style.boxSizing = 'border-box';

	var connectorNameValue = document.createElement('input');
	connectorNameValue.setAttribute('type', 'text');
	connectorNameValue.style.width = '150px';
	connectorNameValue.style.height = '20px';
	connectorNameValue.style.marginLeft = '20px';
	connectorNameValue.style.boxSizing = 'border-box';
	if (properties.get('connectorname') === undefined) {
		connectorNameValue.setAttribute('placeholder', 'connector0');
	} else {
		connectorNameValue.value = properties.get('connectorname');
	}

	connectorName.appendChild(connectorNameValue);

	// synapses 
	// TODO: import local file, then get file name.
	var synapses = document.createElement('div');
	synapses.setAttribute('title', mxResources.get('synapses'));
	mxUtils.write(synapses, mxResources.get('synapses') + ' :');
	synapses.setAttribute('type', 'text');
	synapses.style.marginTop = "6px";
	synapses.style.marginBottom = "6px";
	synapses.style.marginLeft = "25px";
	synapses.style.width = '100%';
	synapses.style.height = '20px';
	synapses.style.boxSizing = 'border-box';

	var synapsesValue = document.createElement('input');
	synapsesValue.setAttribute('type', 'text');
	synapsesValue.style.width = '150px';
	synapsesValue.style.height = '20px';
	synapsesValue.style.marginLeft = '20px';
	synapsesValue.style.boxSizing = 'border-box';
	if (properties.get('synapses') === undefined) {
		synapsesValue.setAttribute('placeholder', '');
	} else {
		synapsesValue.value = properties.get('synapses');
	}
	...
	mxUtils.write(linkType, mxResources.get("linktype") + " :");
	linkType.setAttribute("type", "text");
	linkType.style.marginTop = "6px";
	linkType.style.marginBottom = "6px";
	linkType.style.marginLeft = "25px";
	linkType.style.width = "100%";
	...
	mxUtils.write(linkTypeOption1, 'SPARSE');
	var linkTypeOption2 = document.createElement("option");
	mxUtils.write(linkTypeOption2, 'CONV')
	var linkTypeOption3 = document.createElement("option");
	mxUtils.write(linkTypeOption3, 'TO OUTPUT')
	if (properties.get('linktype') === 'FULL') {
		linkTypeOption0.setAttribute("selected", "selected");
	} else if (properties.get('linktype') === 'SPARSE') {
		linkTypeOption1.setAttribute("selected", "selected");
	} else if (properties.get('linktype') === 'CONV') {
		linkTypeOption2.setAttribute("selected", "selected");
...
DiagramStylePanel.prototype.addView = function (div) {
	...
	var addEntries = mxUtils.bind(this, function () {
		...
		for (var i = this.format.currentStylePage * maxEntries; i < Math.min((this.format.currentStylePage + 1) * maxEntries,
...
DiagramFormatPanel.prototype.init = function () {
	...
	this.container.appendChild(this.addGlobalProperties(this.createPanel()));
	// this.container.appendChild(this.addView(this.createPanel()));
 ...
 * @returns 
 */
DiagramFormatPanel.prototype.addGlobalProperties = function (div) {
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	...
	mxUtils.write(projectName, mxResources.get('projectname') + ' :');
	projectName.setAttribute('title', mxResources.get('projectnamerule'));
	projectName.setAttribute('type', 'text');
	projectName.style.marginTop = "6px";
	projectName.style.marginBottom = "6px";
	projectName.style.marginLeft = "25px";
	projectName.style.width = '100%';
	projectName.style.height = '20px';
	projectName.style.boxSizing = 'border-box';

	var projectNameValue = document.createElement("input");
	projectNameValue.setAttribute("placeholder", "snndemo");
	projectNameValue.setAttribute("type", "text");
	projectNameValue.style.width = "150px";
	projectNameValue.style.height = "20px";
	projectNameValue.style.marginLeft = "20px";
	projectNameValue.style.boxSizing = 'border-box';
	projectNameValue.style.border = "solid";
	projectNameValue.style.borderWidth = "1.5px";
	if (properties.get('projectname') === undefined) {
		projectNameValue.setAttribute("placeholder", "snndemo");
	} else {
		projectNameValue.value = properties.get("projectname");

	}
	projectName.appendChild(projectNameValue)
	// project version
	var projectVersion = document.createElement('div');
	mxUtils.write(projectVersion, mxResources.get('version') + ' :');
	projectVersion.setAttribute('title', mxResources.get('versionrule'));
	projectVersion.setAttribute('type', 'text');
	projectVersion.style.marginTop = "6px";
	projectVersion.style.marginBottom = "6px";
	projectVersion.style.marginLeft = "25px";
	projectVersion.style.width = '100%';
	projectVersion.style.height = '20px';
	projectVersion.style.boxSizing = 'border-box';

	var projectVersionValue = document.createElement('input');
	projectVersionValue.setAttribute('type', 'text');
	projectVersionValue.style.width = '150px';
	projectVersionValue.style.height = '20px';
	projectVersionValue.style.marginLeft = '20px';
	projectVersionValue.style.boxSizing = 'border-box';
	projectVersionValue.style.border = "solid";
	projectVersionValue.style.borderWidth = "1.5px";
	if (properties.get('version') === undefined) {
		projectVersionValue.setAttribute('placeholder', '0.0.1');
	} else {
		projectVersionValue.value = properties.get("version");
	}
	projectVersion.appendChild(projectVersionValue);

	// target architecture
	var targetArch = document.createElement('div');
	mxUtils.write(targetArch, mxResources.get('target') + ' :');
	targetArch.setAttribute('title', mxResources.get('targetrule'));
	targetArch.setAttribute('type', 'text');
	targetArch.style.marginTop = "6px";
	targetArch.style.marginBottom = "6px";
	targetArch.style.marginLeft = "25px";
	targetArch.style.width = '100%';
	targetArch.style.height = '20px';
	targetArch.style.boxSizing = 'border-box';

	var targetArchSelect = document.createElement("select");
	targetArchSelect.style.width = '150px';
	targetArchSelect.style.height = "20px";
	targetArchSelect.style.marginLeft = "20px";
	targetArchSelect.style.border = "solid";
	targetArchSelect.style.borderWidth = "1.5px";
	var taretArchOption0 = document.createElement("option");
	mxUtils.write(taretArchOption0, 'DarwinII');
	var taretArchOption1 = document.createElement("option");
	mxUtils.write(taretArchOption1, 'DarwinIII');
	if (properties.get("targetarch") === 'DarwinII') {
		taretArchOption0.setAttribute("selected", "selected");
	} else if (properties.get("targetarch") === 'DarwinIII') {
		taretArchOption1.setAttribute("selected", "selected");
	}
	targetArchSelect.appendChild(taretArchOption0);
	// targetArchSelect.appendChild(taretArchOption1);
	targetArch.appendChild(targetArchSelect);

	// learn algorithm
	var learnAlgorithm = document.createElement('div');
	mxUtils.write(learnAlgorithm, mxResources.get('learnAlgorithm') + ' :');
	learnAlgorithm.setAttribute('type', 'text');
	learnAlgorithm.style.marginTop = "6px";
	learnAlgorithm.style.marginBottom = "6px";
	learnAlgorithm.style.marginLeft = "25px";
	learnAlgorithm.style.width = '100%';
	learnAlgorithm.style.height = '20px';
	learnAlgorithm.style.boxSizing = 'border-box';

	var learnAlgorithmSelect = document.createElement("select");
	learnAlgorithmSelect.style.width = '150px';
	learnAlgorithmSelect.style.height = "20px";
	learnAlgorithmSelect.style.float = "right";
	learnAlgorithmSelect.style.marginRight = "80px";
	learnAlgorithmSelect.style.border = "soild";
	...
	// learn rate
	var learningrate = document.createElement('div');
	mxUtils.write(learningrate, mxResources.get('learningrate') + ' :');
	learningrate.setAttribute('title', mxResources.get('learningraterule'));
	learningrate.setAttribute('type', 'text');
	learningrate.style.marginTop = "6px";
	learningrate.style.marginBottom = "6px";
	learningrate.style.marginLeft = "25px";
	learningrate.style.width = '100%';
	learningrate.style.height = '20px';
	learningrate.style.boxSizing = 'border-box';

	var learningrateValue = document.createElement("input");
	learningrateValue.setAttribute("placeholder", "0.001");
	learningrateValue.setAttribute("type", "text");
	learningrateValue.style.width = '150px';
	learningrateValue.style.height = '20px';
	learningrateValue.style.boxSizing = 'border-box';
	learningrateValue.style.float = "right";
	learningrateValue.style.marginRight = "80px";
	learningrateValue.style.border = "soild";
	learningrateValue.style.borderWidth = "1.5px";
	if (properties.get('learnRate') === undefined) {
		learningrateValue.setAttribute("placeholder", "0.001");
	} else {
		learningrateValue.value = properties.get("learnRate");
	}
	learningrate.appendChild(learningrateValue)

	// TODO: delay type, get users' setting
	var delayType = document.createElement('div');
	mxUtils.write(delayType, mxResources.get('delaytype') + ' :');
	delayType.setAttribute('title', mxResources.get('delaytype'));
	delayType.setAttribute('type', 'text');
	delayType.style.marginTop = "6px";
	delayType.style.marginBottom = "6px";
	delayType.style.marginLeft = "25px";
	delayType.style.width = '100%';
	delayType.style.height = '20px';
	delayType.style.boxSizing = 'border-box';

	var delayTypeSelect = document.createElement("select");
	delayTypeSelect.style.width = '150px';
	delayTypeSelect.style.height = "20px";
	delayTypeSelect.style.marginLeft = "20px";
	var delayTypeOption0 = document.createElement("option");
	mxUtils.write(delayTypeOption0, '0');
	var delayTypeOption1 = document.createElement("option");
	mxUtils.write(delayTypeOption1, '1');
	var delayTypeOption2 = document.createElement("option");
	mxUtils.write(delayTypeOption2, '2');
	if (properties.get("delaytype") === "0") {
		delayTypeOption0.setAttribute("selected", "selected");
	} else if (properties.get("delaytype") === "1") {
		delayTypeOption1.setAttribute("selected", "selected");
	} else if (properties.get("delaytype") === "2") {
		delayTypeOption2.setAttribute("selected", "selected");
	}
	delayTypeSelect.appendChild(delayTypeOption0);
	delayTypeSelect.appendChild(delayTypeOption1);
	delayTypeSelect.appendChild(delayTypeOption2);
	delayType.appendChild(delayTypeSelect);

	// leak sign
	var leakSign = document.createElement('div');
	mxUtils.write(leakSign, mxResources.get('leaksign') + ' :');
	leakSign.setAttribute('title', mxResources.get('leaksignrule'));
	leakSign.setAttribute('type', 'text');
	leakSign.style.marginTop = "6px";
	leakSign.style.marginBottom = "6px";
	leakSign.style.marginLeft = "25px";
	leakSign.style.width = '100%';
	leakSign.style.height = '20px';
	leakSign.style.boxSizing = 'border-box';

	var leakSignSelect = document.createElement("select");
	leakSignSelect.style.width = '150px';
	leakSignSelect.style.height = "20px";
	leakSignSelect.style.marginLeft = "20px";
	leakSignSelect.style.border = "solid";
	leakSignSelect.style.borderWidth = "1.5px";
	var leakSignOption0 = document.createElement("option");
	mxUtils.write(leakSignOption0, 'yes');
	var leakSignOption1 = document.createElement("option");
	mxUtils.write(leakSignOption1, 'no');
	if (properties.get("leaksign") === "yes") {
		leakSignOption0.setAttribute("selected", "selected");
	} else if (properties.get("leaksign") === "no") {
		leakSignOption1.setAttribute("selected", "selected");
	}
	leakSignSelect.appendChild(leakSignOption0);
	leakSignSelect.appendChild(leakSignOption1);
	leakSign.appendChild(leakSignSelect);

	// div.appendChild(projectName);
	// div.appendChild(projectVersion);
	// div.appendChild(targetArch);
	div.appendChild(learnAlgorithm);
	div.appendChild(learningrate);

	names = ['label','learnAlgorithm', 'learnRate'];
	var applyBtn = mxUtils.button(mxResources.get('apply'), function () {
		try {
			texts.push("globalvalue");
			// texts.push(targetArchSelect.options[targetArchSelect.selectedIndex].value);
			texts.push(learnAlgorithmSelect.options[learnAlgorithmSelect.selectedIndex].value);
			texts.push(learningrateValue.value ===""?"0.001":learningrateValue.value);
			// texts.push('[0, 0]');
			// texts.push('globalvalue');
